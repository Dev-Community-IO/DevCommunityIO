...
export function OpportunityDetail({ opportunityId, onClose }: OpportunityDetailProps) {
  ...
  const [pageData, setPageData] = useState<Page | null>((opportunity as any)?.post?.page || null);
  ...
  useEffect(() => {
    const fetchOpportunity = async () => {
      try {
        ...
        if ((opportunityData as any)?.post?.page) {
          const page = (opportunityData as any).post.page as any;
          setPageData({
            ...page,
            isFollowing: page?.isFollowing === true
          } as Page);
        } else if ((opportunityData as any)?.post?.pageId) {
          try {
            const pageResponse = await pagesService.getPage((opportunityData as any).post.pageId);
  ...
  return (
    <>
      ...
      {(opportunity as any)?.post?.author && (
        <GlassCard className="p-4 sm:p-6">
          <div className="flex items-start gap-3 sm:gap-4">
            <div className="flex-shrink-0">
              {(opportunity as any).post.author && (
                <Tooltip content={`@${(opportunity as any).post.author.username || (opportunity as any).post.author.pseudo || 'unknown'}`}>
                  <div 
                    onClick={() => navigate(`/profile/${(opportunity as any).post.author.username || (opportunity as any).post.author.pseudo || ''}`)}
                    className="relative cursor-pointer hover:scale-105 transition-transform"
                  >
                    <Avatar 
                      src={((opportunity as any).post.author.avatar || (opportunity as any).post.author.avatarUrl) || ''} 
                      alt={(opportunity as any).post.author.username || (opportunity as any).post.author.pseudo || 'Unknown'} 
                      size="md" 
                      ...
                      isTrusted={(opportunity as any).post.author.isTrusted}
                    />
            ...
            <div className="flex-1 min-w-0">
              {(opportunity as any).post.author && (
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  >
                    {(opportunity as any).post.author.username || (opportunity as any).post.author.pseudo || 'Unknown'}
                  </span>
                  {(opportunity as any).post.author.isVerified && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
        ...
        <div className="lg:col-span-2 space-y-4 sm:space-y-6">
          ...
          <GlassCard className="p-4 sm:p-6 lg:p-8">
            ...
            {/* Tags */}
            {(opportunity.post as any)?.tags && (opportunity.post as any).tags.length > 0 && (
              <div className="flex flex-wrap gap-2 mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
                {(opportunity.post as any).tags.map((tag: any) => {
