    ...
    >
      <div className="flex flex-col space-y-2 sm:space-y-2.5">
            ...
            <div className="flex items-center gap-1.5 min-w-0">
              {organizer && (
                <>
                  ...
                  {!hackathon.post?.page && (
                    <ReputationBadge value={organizer.reputation || 0} />
                  )}
                </>
              )}
              <span className={contentTimestampClass}>•</span>
              <span className={contentTimestampClass}>{timeAgo(hackathon.createdAt)}</span>
            </div>
          </div>
          <button
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          {hackathon.category && <span className={`${postTagClass} capitalize`}>{hackathon.category}</span>}
          {hackathon.difficulty && (
            <span className={`${postTagClass} capitalize`}>{hackathon.difficulty}</span>
          )}
        </div>

        <h3 className={contentFeedTitleClass}>{hackathon.title}</h3>
        ...
        {/* Description Preview */}
        <p className={contentFeedPreviewClass}>{getCleanPreview(hackathon.description, 200)}</p>

        <div className="flex flex-wrap items-center gap-1.5">
          {hackathon.startDate && (
            <span className={contentMetaChipClass}>
              <Calendar size={12} strokeWidth={2} className="text-zinc-400" />
              {formatDate(hackathon.startDate)}
            </span>
          )}
          {hackathon.participantCount !== undefined && (
            <span className={contentMetaChipClass}>
              <Users size={12} strokeWidth={2} className="text-zinc-400" />
